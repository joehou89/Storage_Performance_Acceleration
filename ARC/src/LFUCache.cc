...
void LFUCache::cache_insert(DataType data) {
    if (_capacity == 0) return; // 不能加入元素
    if (_map.find(data)!= _map.end()) {
        // 列表中有该元素
        CacheList* L = _map[data];
        detach_listnode(L, false);
        L->time += 1;   // 增加使用次数标记
        insert(L, false);
        ...
        if (_size == _capacity) {
            delete_tail_listnode(false); // 删除一个元素并加入ghost中
        }
        L->key = data;
        L->time = _transform_time;
        
        cout << "putting : " << L->key << "the time is " << _transform_time << endl;
        _map[data] = L;
        insert(L, false);
    }
}
...
void LFUCache::insert(CacheList* L, bool is_ghost) {
    if (!is_ghost) {
        ...
        if (!_cache) { // 空则建立结点
        ...
        else if (_cache->time < L->time) { // 插入到头结点上
            CacheList* p = _cache->prev;
            L->next = _cache;
            L->prev = p;
            p->next = L;
            _cache->prev = L;
            ...
            CacheList* pre = _cache;
            // 在循环链表中有序插入元素 -> 此处有不足，如果使用双哈希表可以得到O(1)的复杂度
            for (; pre->next != _cache && pre ->next-> time > L->time; pre = pre->next);
...
// 添加元素并且增加空间
void LFUCache::cache_insert_and_extend(DataType data) {
    _capacity += 1;
    CacheList* L = new CacheList();
    L->key = data;
    L->time = 1;  // 此时调用的次数设为1
...
bool LFUCache::cache_check_ghost(DataType data) {
